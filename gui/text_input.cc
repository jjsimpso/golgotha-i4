...
{        
  ...
  for (x=0; x<width() && c!=st->end(); )
  {
    ...
    if (c!=st->end())
    {
      ...
      if (cursor_on && (w32)cursor_x==char_on)
        local_image->bar(cx1+x,cy1+1,
    ...
    else if (cursor_on && (w32)cursor_x==char_on)
...
w32 i4_text_input_class::mouse_to_cursor()
{
  ...
  for (sw32 x=0; x<left_x && c!=st->end(); x++)
  {
    ++c;
    char_on++;
  }
  ...
  w32 fw;
  for (w32 w=0; w<width() && c!=st->end(); )
  {
    fw=font->width(c.get());
    if (last_x<=sw32((w+fw/2+1)))
      return char_on;
