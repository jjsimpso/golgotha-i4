...
{
  w32 sig=in->read_32(); 
  if (sig!=i4_check_sum32((void*)"GOLG_SECTION_ID=32",17))
  ...
  while (copy_size)
  {
    int rs = int(sizeof(buf)) > copy_size ? copy_size : sizeof(buf);
    in->read(buf, rs);
    out->write(buf, rs);
    copy_size-=rs;
  }
  ...
  for (i=0; i<total_sections_to_insert; i++)
  {
    ...
    while (fsize)
    {
      int rs = int(sizeof(buf)) > fsize ? fsize : sizeof(buf);
    ...
    close_on_delete(close_on_delete)
{
  ...
  if (sig==i4_check_sum32((void*)"GOLG",4))
    use_16=1;
  else if (sig==i4_check_sum32((void*)"GOLG_SECTION_ID=32",17))
    use_16=0;
